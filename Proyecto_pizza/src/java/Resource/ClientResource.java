...
public class ClientResource {
   ...
   @Path("/{ID}")    
    public String getJson(@PathParam("ID") int ID) {
        Gson gson = new Gson();
        PersonaDAO  respond= new PersonaDAO();
        Persona buscada=  respond.Select_persona(ID);
        System.out.println("edad" +buscada.getFecha_de_nacimiento().toString());
        return gson.toJson(buscada);
